...
def count_GS_NP():
    """To count the number of Gold Standard NP in the original train DataSet

    GS_NP is the global variable which holds the number of P in original DataSet
    """
    global GS_NP
    for line in input_:
...
def system_precision(precision_value):
    ...
    for line in input_:
        ...
        values = line.split("\t")
        # Since the value is in string format so must be typecasted to float
        temp = float(values[1].rstrip('\n'))
        if(values[0] == 'NP' and temp >= precision_value):
            Common_NP += 1
        if (temp >= precision_value):
            MS_NP += 1
    PRECISION = float(Common_NP)*100 / MS_NP
...
def system_recall(precision_value):
    ...
    Arguments:
        ...
        precision_value  -- Parameter that is the factor to determine P in our system. Above this precision_value, how many values will be there, are P for my system.
    """
    RECALL = float(Common_NP)*100 / count_GS_NP()
    # Rounding is used to get precision upto 2 digits
